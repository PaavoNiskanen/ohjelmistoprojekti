...
PLAYER_DEFAULT_COLLISION_FACTOR = 0.2
PLAYER_MIN_COLLISION_RADIUS = 8


class Player(pygame.sprite.Sprite):
    """
    Pelaajaluokka - yhdistää pygame.sprite.Sprite:n ja fysiikan.
    
    Lataa spritet dynaamisesti aluksen nimen perusteella.
    Tuetut kansiorakenteet:
        - alukset/alus/<SHIP>/Move/*.png
        - alukset/alus/<SHIP>/Boost/*.png
        - alukset/alus/<SHIP>/Destroyed/*.png
        - alukset/<SHIP>-SPRITES/Fly1_*.png, Boost_*.png jne.
    
    Ominaisuudet:
        - Pelaaja-liikunta (W/A/D/S)
        - Boost-mekanismi (Shift)
        - Ampuminen (L/P)
        - Animaatiot (idle, move, boost, destroyed, hurt)
        - Box2D fysiikka (valinnainen)
    """
    ...
    def __init__(self, ship_name, scale_factor, x, y, max_health: int = 5):
        ...
        try:
            self.collision_radius = max(
                PLAYER_MIN_COLLISION_RADIUS,
                int(max(self.rect.width, self.rect.height) * PLAYER_DEFAULT_COLLISION_FACTOR)
            )
        except Exception:
        ...
        # Armor ja damage bonuksit
        self.armor = 0  # Armor points (reduces incoming damage)
        self.damage_bonus = 0  # Damage multiplier (+1 = +100%)
        self.speed_boost_multiplier = 1.0  # Movement speed multiplier (1.0 = normal)
    ...
    def update(self, dt):
        ...
        if hasattr(self, 'speed_boost_multiplier') and self.speed_boost_multiplier > 1.0:
            # Speed boost active, gradually decrease it
            self.speed_boost_multiplier = max(1.0, self.speed_boost_multiplier - dt_s * 0.1)
    ...
    def handle_movement(self, dt):
        ...
        if self.input.moveUp:
            ...
            # Speed boost powerup multiplies max speed (+25%)
            base_max_speed = 1000.0 if self.boost_active else 400.0
            max_speed = base_max_speed * getattr(self, 'speed_boost_multiplier', 1.0)
